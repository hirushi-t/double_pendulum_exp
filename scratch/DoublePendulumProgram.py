...
def record_video(video_length, width, height, fps, camera_num, filename=filename):
    '''
    Records a video using the specified camera and saves it to a file.

    Args:
        video_length (int or float): Duration of the recording in seconds.
        width (int): Width of the video frame in pixels.
        height (int): Height of the video frame in pixels.
        fps (int or float): Frames per second for the recording.
        camera_num (int): Index of the camera to use.
        filename (str): Name of the output file (without extension).

    Description:
        This function initializes the specified camera using Picamera2,
        configures it with the given resolution and frame rate, applies
        manual exposure and analogue gain settings, and records a video
        for the specified duration. The video is encoded using H264Encoder
        and saved to the provided filename.
    '''
...
def init_frame_extraction(filename=filename):
    '''
    Args: where to get file from
    Return: cap - a VideoCapture object set to extract frames from the file selected
    '''
    
    # create a VideoCapture object to open the input video file
    cap = cv2.VideoCapture(filename + ".h264")
   ...
   # check if camera opened successfully 
	...
	# this object is needed to read frames from the video
    return cap
...
def extract_next_frame(cap):
	...
	# ret is True/False --> whether frame is read successfully
...
def find_centre(img):
	# detects 2 perpendicular cross-hair lines in the image using the Hough transform
	# determines how much the image needs to be rotated to align them vertically/horizontally
    ...
    # threshold the image
    # if pixel value > 50, set to 255 (white)
    # if pixel value < 50, set to 0 (black)
    ...
    # Uses Hough Transform to detect straight lines
...
def angle_vs_time(total_frames, centre, rotation, cap, fps, show_video, show_graph):
	# processes each frame to track two blobs (the pendulum masses)
	# for each frame, it rotates the cross-hairs to be the 'right' way up
	# detect the 2 blobs
	# computed their angles relative to the centre
	# optionaly displays the processed video and/or the animated angle vs time graph
